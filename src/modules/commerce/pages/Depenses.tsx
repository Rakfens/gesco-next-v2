...
export default function Depenses() {
  ...
  const [saving, setSaving] = useState<boolean>(false); // Pour les actions de sauvegarde (create/update)
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      if (table === 'depenses') { // Ecouter les changements sur la table 'depenses'
        loadDepenses();
      }
    };
  ...
  const loadDepenses = useCallback(async () => {
    ...
    try {
      ...
      console.error("Erreur chargement des dépenses:", error);
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      console.error("Erreur suppression de la dépense:", error);
  ...
  return (
    ...
    <Button variant="danger" onClick={handleSubmit} disabled={saving} loading={saving}> {/* Ajout de 'loading={saving}' */}
    ...
    <div>
    <h1 style={{ fontSize: 22, fontWeight: 800, color: 'var(--text)' }} data-testid="page-title">Dépenses</h1>
    <p style={{ color: 'var(--muted)', fontSize: 13, marginTop: 4 }}>{currentCompany?.name} · {depenses.length} enregistrement(s)</p>
    </div>
    ...
    {/* Table */}
    {loading ? (
      <div style={{ textAlign: 'center', color: 'var(--muted)', padding: 40, fontSize: 14 }}>Chargement...</div>
    ) : (
      <Table>
      <TableHead>
      ...
      {depenses.map(d => (
        ...
        <Button variant="danger" size="sm" onClick={() => setConfirmDelete(d)}>Supprimer</Button>
        </TableCell>
        </TableRow>
      ))}
      </TableBody>
