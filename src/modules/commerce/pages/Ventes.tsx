...
export default function Ventes() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.error("Erreur lors du chargement des ventes/produits:", error);
  ...
  const handleEdit = async (vente: Vente) => {
    ...
    try {
      ...
      if (v?.details) {
        setPanier(v.details.map((d: any) => ({ // Typage temporaire 'any' pour vente_details
  ...
  const handlePrint = async (venteId: string) => {
    try {
      ...
      if (v) printTicketVente(v as any, v.details as any, currentCompany as any); // Typage temporaire 'any'
