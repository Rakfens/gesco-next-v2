        ...
        return (
          <div key={item.date} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 44, flex: '0 0 auto' }}>
            ...
            <div style={{ width: '100%', background: 'var(--bg)', borderRadius: '4px 4px 0 0', height: 84, display: 'flex', alignItems: 'flex-end' }}>
              <div style={{ width: '100%', height: h + '%', background: color, borderRadius: '4px 4px 0 0', transition: 'height 0.4s ease', minHeight: 3 }} />
            </div>
...
const PERIOD_OPTIONS = [
  ...
  { value: 'semaine', label: 'Semaine', getDates: () => { const today = new Date(); const day = today.getDay(); const diff = day === 0 ? 6 : day - 1; const first = new Date(today); first.setDate(today.getDate() - diff); return { debut: first.toISOString().split('T')[0], fin: today.toISOString().split('T')[0] }; } },
  ...
  { value: 'trimestre', label: 'Trimestre', getDates: () => { const now = new Date(); const q = Math.floor(now.getMonth() / 3); return { debut: new Date(now.getFullYear(), q * 3, 1).toISOString().split('T')[0], fin: now.toISOString().split('T')[0] }; } },
...
export default function Rapports() {
  ...
  if (loading) return <div style={{ color: 'var(--muted)', padding: 60, textAlign: 'center', fontSize: 14 }}>Chargement des rapports...</div>;

  return (
    <div style={{ padding: '0 0 24px' }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: 'var(--text)' }} data-testid="page-title">Rapports</h1>
        <p style={{ color: 'var(--muted)', fontSize: 13, marginTop: 4 }}>{currentCompany?.name} · Analyse des performances</p>
      </div>

      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ display: 'flex', gap: 4, background: 'var(--card)', borderRadius: 10, padding: 4, border: '1px solid var(--border)', flexWrap: 'wrap' }}>
          {PERIOD_OPTIONS.map(opt => (
            <button key={opt.value} onClick={() => handlePeriodChange(opt.value)} style={{
              padding: '6px 14px', border: 'none', borderRadius: 7, cursor: 'pointer', fontSize: 12, fontWeight: 600, fontFamily: 'var(--font)',
              background: period === opt.value ? 'var(--accent)' : 'transparent', color: period === opt.value ? '#fff' : 'var(--text2)',
            }}>{opt.label}</button>
          ))}
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <input type="date" value={dateDebut} onChange={e => setDateDebut(e.target.value)} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid var(--border2)', background: 'var(--card)', color: 'var(--text)', fontSize: 13 }} />
          <span style={{ color: 'var(--muted)' }}>vers</span>
          <input type="date" value={dateFin} onChange={e => setDateFin(e.target.value)} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid var(--border2)', background: 'var(--card)', color: 'var(--text)', fontSize: 13 }} />
          <button onClick={loadReports} style={{ padding: '8px 14px', borderRadius: 8, border: 'none', background: 'var(--accent)', color: '#fff', cursor: 'pointer', fontSize: 14 }}>Actualiser</button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 12, marginBottom: 20 }}>
        ...
        <StatCard label="Marge brute" value={formatAr(stats.marge)} color={stats.marge >= 0 ? "var(--purple)" : "var(--red)"} sub={margeSub} />
        <StatCard label="Produits" value={stats.nbProduits} color="var(--teal)" />
        <StatCard label="Alertes stock" value={stats.alertesStock} color={stats.alertesStock > 0 ? "var(--red)" : "var(--green)"} sub={alertesSub} />
        {currentCompany?.slug === 'pomanay' && <StatCard label="Dépenses" value={formatAr(stats.totalDepenses)} color="var(--pink)" />}
      </div>
      ...
      {topProduits.length > 0 && (
        <Card style={{ marginBottom: 16 }}>
          <CardHeader><CardTitle>Top 10 produits</CardTitle></CardHeader>
          <Table>
            <TableHead>
            ...
            <TableBody>
              {topProduits.map((p, idx) => (
                <TableRow key={idx}>
                  <TableCell style={{ fontWeight: 700, color: idx < 3 ? 'var(--orange)' : 'var(--muted)' }}>{idx + 1}</TableCell>
                  <TableCell style={{ fontWeight: 600 }}>{p.produit_nom || p.produit?.nom || '—'}</TableCell>
                  <TableCell align="right">{p.quantite}</TableCell>
                  <TableCell align="right" style={{ fontWeight: 700, color: 'var(--green)' }}>{formatAr(p.chiffre)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
              ...
              return (
                <div key={cat} style={{ marginBottom: 12 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    ...
                    <span style={{ fontSize: 13, color: 'var(--text2)' }}>{formatAr(total)} · {pct.toFixed(1)}pct</span>
                  </div>
                  <div style={{ background: 'var(--bg)', height: 6, borderRadius: 3, overflow: 'hidden' }}>
                    <div style={{ width: pct + '%', background: 'var(--pink)', height: '100%', borderRadius: 3 }} />
                  </div>
                </div>
              );
            })}
