...
interface PomanayStats {
  caMois: number;
  achatsMois: number;
  depensesMois: number;
}

const renderPomanaySpecificStats = (benefice: number, stats: PomanayStats) => (
  <Card
    padding={20}
    style={{
      marginBottom: 24,
  ...
  >
    ...
    <div
      style={{
        fontSize: 32,
        fontWeight: 800,
        color: benefice >= 0 ? "var(--green)" : "var(--red)",
      }}
    >
      {formatAr(benefice)}
    </div>
    <CardContent style={{ fontSize: 12, color: "var(--muted)", marginTop: 6, padding: 0 }}>
      CA {formatAr(stats.caMois)} — Achats {formatAr(stats.achatsMois)} — Dépenses{" "}
      {formatAr(stats.depensesMois)}
    </CardContent>
  </Card>
);
...
export default function CommerceDashboard() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (currentCompany.slug === "pomanay") {
        ...
        depensesJour = (dj || []).reduce(
          (s: number, d: { montant?: number }) => s + (d.montant || 0),
          0,
        );
        depensesMois = (dm || []).reduce(
          (s: number, d: { montant?: number }) => s + (d.montant || 0),
          0,
        );
      }
  ...
  }, [currentCompany, load]); // Dépend de currentCompany

  useEffect(() => {
    const h = (e: Event) => {
      if (["ventes", "achats", "produits", "depenses"].includes((e as CustomEvent).detail?.table))
        load();
    };
    window.addEventListener("supabase_realtime", h);
    return () => window.removeEventListener("supabase_realtime", h);
  }, [load]); // Dépend de currentCompany pour la cleanup
  ...
  return (
    <div style={{ paddingBottom: 24 }}>
      ...
      >
        ...
        {pomanayExtra && (
          <StatCard
            ...
            value={formatAr(stats.depensesJour)}
            color="var(--red)"
          />
        )}
        {pomanayExtra && (
          <StatCard
            ...
            value={formatAr(stats.depensesMois)}
            color="var(--orange)"
          />
        )}
      </div>
      ...
      {alertes.length > 0 && (
        ...
        >
          <CardTitle style={{ color: "var(--orange)" }}>Stock bas ({alertes.length})</CardTitle>
      ...
      <Card padding={0}>
        <CardHeader style={{ padding: "14px 18px", borderBottom: "1px solid var(--border)" }}>
          ...
          <span style={{ fontSize: 12, color: "var(--muted)" }}>
            {recentVentes.length} transaction(s)
          </span>
        </CardHeader>
        <Table>
          <TableHead>
          ...
          <TableBody>
            {recentVentes.length === 0 ? (
              ...
              recentVentes.map((v) => (
                <TableRow key={v.id}>
                  <TableCell style={{ fontWeight: 600, fontFamily: "var(--font-mono)" }}>
                    {v.numero_facture}
                  </TableCell>
                  ...
                  <TableCell align="right" style={{ fontWeight: 600, color: "var(--green)" }}>
