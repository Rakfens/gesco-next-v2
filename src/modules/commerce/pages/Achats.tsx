...
export default function Achats() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.error("Erreur lors du chargement des achats/produits:", error);
  ...
  const produitsFiltres = useMemo(() => {
    ...
    !searchProduit || p.nom.toLowerCase().includes(q) || (p.reference || '').toLowerCase().includes(q)
    );
  ...
  return (
    ...
    {panier.length === 0
      ...
      : panier.map(item => (
        ...
        style={{ width: 90, padding: '3px 7px', fontSize: 12 } as React.CSSProperties}
        />
        </div>
    ...
    onChange={e => setForm({ ...form, montant_paye: parseFloat(e.target.value) || 0 })}
    />
    </div>

    {/* Récap */}
    <div style={{ background: 'var(--bg)', padding: 14, borderRadius: 12, marginTop: 14 }}>
    {[
    ...
    <Button variant="warning" onClick={handleSubmitAchat} disabled={saving} loading={saving}> {/* Ajout de 'loading={saving}' */}
