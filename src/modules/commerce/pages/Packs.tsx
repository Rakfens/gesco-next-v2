...
export default function PacksPage() {
  ...
  return (
    ...
    <Icon d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" size={18} className="text-primary" />
    </div>
    <div>
    ...
    {currentCompany?.name} · {packs.length} pack(s)
    </p>
    </div>
    </div>
    <div className="flex gap-2">
    <Button variant="secondary" size="sm" onClick={() => router.push("/commerce/ventes")} className="btn-press">
    ← Ventes
    </Button>
    <Button variant="primary" onClick={openCreateModal} className="btn-press shadow-gold">
    ＋ Nouveau pack
    </Button>
    </div>
    </div>
    ...
    <StatCard
    label="Marge totale"
    value={formatAr(stats.margeTotal)}
    color="accent"
    ...
    {(["tous", "disponible", "indisponible"] as const).map((f) => (
      ...
      className={`py-1.5 px-3 rounded-lg border text-[11px] font-semibold cursor-pointer transition-colors btn-press ${filterDisponible === f ? "bg-primary text-background border-primary" : "bg-transparent text-muted-foreground border-border hover:text-foreground"}`}
      >
    ...
    {packsFiltres.length === 0 ? (
      ...
      {packsFiltres.map((pack) => (
        <Card key={pack.id} padding={0} className="overflow-hidden glass-card animated-border">
        <div className="py-4 px-5">
        <div className="flex items-center justify-between flex-wrap gap-2.5">
        <div className="flex items-center gap-3">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${pack.disponible ? "bg-success/10" : "bg-destructive/10"}`}>
        <span className="text-[22px]">{pack.disponible ? "📦" : "⚠️"}</span>
        </div>
        <div>
        <div className="flex items-center gap-2">
        <span className="font-bold text-[15px] text-foreground">{pack.nom}</span>
        <Badge
        variant={pack.disponible ? "success" : "danger"}
        size="sm"
        className={pack.disponible ? "glow-success" : "glow-danger"}
        >
        ...
        <div className="text-lg font-extrabold text-primary">{formatAr(pack.prix)}</div>
        <div className="text-[10px] text-muted-foreground">
        Achat: {formatAr(pack.valeurAchat || 0)} · Marge:{" "}
        <span className={(pack.marge || 0) >= 0 ? "text-success" : "text-destructive"}>
        {formatAr(pack.marge || 0)}
        </span>
        </div>
        </div>
        <div className="flex gap-1.5">
        <Button variant="success" size="sm" onClick={() => router.push("/commerce/ventes")} className="btn-press">
        🛒 Vendre
        </Button>
        <Button variant="secondary" size="sm" onClick={() => openDetailModal(pack.id)} className="btn-press">
        👁️
        </Button>
        <Button variant="primary" size="sm" onClick={() => openEditModal(pack)} className="btn-press">
        ✏️
        </Button>
        <Button variant="danger" size="sm" onClick={() => setShowDeleteModal(pack.id)} className="btn-press">
        🗑️
        </Button>
        </div>
        </div>
    ...
    {formProduits.map((fp, index) => (
      ...
      className="flex-1 py-2 px-3 bg-background border border-border rounded-lg text-foreground text-[13px] outline-none font-sans input-focus"
      >
      ...
      className="w-[60px] py-2 px-2 bg-background border border-border rounded-lg text-foreground text-[13px] text-center outline-none input-focus"
      />
      <button
      onClick={() => removeProduitLine(index)}
      className="w-8 h-8 rounded-lg bg-destructive/10 border border-destructive/20 text-destructive cursor-pointer flex items-center justify-center hover:bg-destructive/20 transition-colors btn-press"
      >
      ✕
      </button>
      </div>
    ...
    <Button variant="secondary" size="sm" onClick={addProduitLine} className="mb-2 btn-press">
    ＋ Ajouter un produit
    </Button>
    </ModalBody>
    <ModalFooter>
    ...
    <ModalBody>
    {detailLoading ? (
      <div className="text-center p-10 text-muted-foreground animate-fade-in">Chargement...</div>
    ) : detailPack ? (
      <>
      <div className="grid grid-cols-2 gap-2.5 mb-4">
      <div className="py-2.5 px-3.5 bg-background rounded-lg">
      <div className="text-[10px] text-muted-foreground mb-0.5">Prix de vente</div>
      <div className="text-lg font-extrabold text-primary">{formatAr(detailPack.prix)}</div>
      </div>
      <div className="py-2.5 px-3.5 bg-background rounded-lg">
      <div className="text-[10px] text-muted-foreground mb-0.5">Nombre de produits</div>
      <div className="text-lg font-extrabold text-info">{detailPack.produits?.length || 0}</div>
      </div>
      </div>
      <div className="grid grid-cols-2 gap-2.5 mb-4">
      <div className="py-2.5 px-3.5 bg-background rounded-lg">
      <div className="text-[10px] text-muted-foreground mb-0.5">Valeur d'achat</div>
      <div className="text-base font-bold text-warning">{formatAr(detailPack.valeurAchat || 0)}</div>
      </div>
      <div className="py-2.5 px-3.5 bg-background rounded-lg">
      <div className="text-[10px] text-muted-foreground mb-0.5">Marge</div>
      <div className={`text-base font-bold ${(detailPack.marge || 0) >= 0 ? "text-success" : "text-destructive"}`}>
      {formatAr(detailPack.marge || 0)}
      </div>
      </div>
      </div>
      {detailPack.description && (
        <div className="py-2.5 px-3.5 bg-background rounded-lg mb-4">
        <div className="text-[10px] text-muted-foreground mb-0.5">Description</div>
        <div className="text-[13px] text-foreground">{detailPack.description}</div>
        </div>
      )}
      <div className={`py-2.5 px-3.5 rounded-lg mb-4 flex items-center gap-2 ${detailPack.disponible ? "bg-success/10" : "bg-destructive/10"}`}>
      <span className="text-base">{detailPack.disponible ? "✅" : "❌"}</span>
      <span className={`text-[13px] font-semibold ${detailPack.disponible ? "text-success" : "text-destructive"}`}>
      ...
      <div className="text-xs font-bold text-foreground mb-2">Composition du pack</div>
      <Table>
      <TableHead>
      <TableRow>
      ...
      {detailStock.map((ds, idx) => (
        ...
        <TableCell className="font-semibold text-foreground">{ds.nom}</TableCell>
        <TableCell align="center" className="text-foreground">{ds.quantite}</TableCell>
        <TableCell align="right" className={ds.suffisant ? "text-foreground" : "text-destructive"}>
        {ds.stock}
        </TableCell>
        <TableCell align="center">
        <Badge variant={ds.suffisant ? "success" : "danger"} size="sm" className={ds.suffisant ? "glow-success" : "glow-danger"}>
      ...
      <div className="text-center p-10 text-muted-foreground">Pack introuvable</div>
    )}
    </ModalBody>
    </Modal>
    ...
    <Button variant="danger" onClick={handleDelete} loading={saving} className="btn-press">Supprimer</Button>
    </ModalFooter>
    </Modal>
    </div>
  );
