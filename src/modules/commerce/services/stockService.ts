...
export const createMouvementProduit = async (
  mouvementData: MouvementStockData,
): Promise<Produit> => {
...
export const fetchMouvementsProduit = async (
  filters: MouvementFilters = {},
): Promise<MouvementWithProduit[]> => {
  const company = getCurrentCompany();
  if (!company) return [];
...
export const createMouvementStockManuel = async (
  mouvementData: MouvementStockData,
): Promise<Produit> => {
  const company = getCurrentCompany();
  if (!company) throw new Error("Aucune société sélectionnée");
...
export const getStockBas = async (): Promise<Record<string, unknown>[]> => {
  ...
  return (data || []).filter(
    (p: Record<string, unknown>) =>
      (p.quantite_stock as number) <= ((p.stock_minimum as number) || 0),
  );
};
...
export const getRotationProduit = async (
  ...
  const quantiteVendue = (ventes || []).reduce(
    (sum: number, v: { quantite: number }) => sum + v.quantite,
    0,
  );
