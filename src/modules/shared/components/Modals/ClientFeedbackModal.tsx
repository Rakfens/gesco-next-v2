// ClientFeedbackModal.tsx — v5 : sticky footer (boutons toujours visibles mobile)
...
export const ClientFeedbackModal = ({
  ...
  const StatBadge = ({ statut }: { statut: string }) => {
    const s = (STATUTS as Record<string, { label: string; color: string; bg: string }>)[statut];
    if (!s) return null;
    return (
      <span
        style={{
          background: s.bg || "var(--card2)",
        ...
        <div style={modalStyles.body}>
          ...
          >
            {[
              {
                label: "Livrés",
                count: livsLivrees.length,
                color: "var(--green)",
                bg: "var(--green-dim)",
              },
              {
                label: "Retournés",
                count: livsRetournees.length,
                color: "var(--red)",
                bg: "var(--red-dim)",
              },
              {
                label: "Reportés",
                count: livsReportees.length,
                color: "var(--orange)",
                bg: "var(--orange-dim)",
              },
              {
                label: "En cours",
                count: livsEnCours.length,
                color: "var(--blue)",
                bg: "var(--blue-dim)",
              },
            ].map((s) => (
              ...
              {livsLivrees.map((l) => (
                ...
                >
                  ...
                  <span
                    style={{
                      fontWeight: 700,
                      color: l.paiement === "client" ? "var(--blue)" : "var(--green)",
                    }}
                  >
                    {l.paiement === "client"
                      ? "Payé client"
          ...
          >
            ...
            >
              ...
              <span style={{ color: aVerser >= 0 ? "var(--green)" : "var(--red)" }}>
                {formatAr(aVerser)}
              </span>
            </div>
          </div>
