...
export const getCompanyMeta = (company: Company | null): CompanyMeta => {
  ...
  if (company.slug === "pomanay")
    return { label: "Boutique", color: "var(--purple)", icon: "POM", bg: "var(--purple-dim)" };
  if (company.slug === "zazatiana")
    return { label: "Bébé", color: "var(--pink)", icon: "ZAZ", bg: "rgba(244,114,182,0.12)" };
  return { label: "Livraison", color: "var(--blue)", icon: "LIV", bg: "var(--blue-dim)" };
};
