...
export default function Recuperation() {
  const { agents, showToast } = useApp(); // Supposons que agents est correctement typé comme Agent[]
  // Plus besoin de casting
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      console.error("Erreur lors de l'ajout de la récupération:", error);
  ...
  const handleUpdate = async () => {
    ...
    if (editData.frais_recuperation <= 0) { // Validation supplémentaire
      showToast('Le frais de récupération doit être supérieur à 0', 'error');
      return;
    }
    try {
      ...
      console.error("Erreur lors de la modification de la récupération:", error);
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      console.error("Erreur lors de la suppression de la récupération:", error);
      showToast("Erreur lors de la suppression de la récupération.", 'error');
    }
  };

  // Calcul des totaux
  const totalGains = useMemo(() => recuperations.reduce((s, r) => s + (r.frais_recuperation || 0), 0), [recuperations]);
  ...
  return (
    ...
    placeholder="1000"
    value={form.frais_recuperation} // Utiliser le nombre directement
    onChange={e => setForm({ ...form, frais_recuperation: Number(e.target.value) || 0 })} // Conversion explicite
      ...
      {Object.values(recuperationsParLivreur).map(rl => (
        ...
        {rl.recuperations.map(r => (
          ...
          {editId === r.id ? (
            ...
            type="number"
            placeholder="Frais"
            value={editData.frais_recuperation} // Utiliser le nombre
            onChange={e => setEditData({ ...editData, frais_recuperation: Number(e.target.value) || 0 })} // Conversion explicite
            style={{ width: 150 }}
