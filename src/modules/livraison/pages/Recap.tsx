...
export default function Recap() {
  const {
    ...
    agents,
    showToast,
    addAvance: onAddAvance,
    deleteAvance: onDeleteAvance,
  } = useApp();
  ...
  const { monthLivs, monthAvances } = useMemo(() => {
    const monthLivs = livraisons.filter((l) => l.date?.startsWith(selectedMonth));
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        logger.error("Erreur chargement recuperations:", error);
        if (showToast) {
  ...
  const {
    ...
    const monthBenefice =
      monthTotalFrais - monthTotalSalaires - monthGerantGain - monthTotalRecuperations;
  ...
  const handleAddAvance = async () => {
    ...
    try {
      ...
      logger.error("Erreur lors de l'ajout de l'avance:", error);
  ...
  const executeDeleteAvance = async () => {
    ...
    try {
      ...
      logger.error("Erreur lors de la suppression de l'avance:", error);
    ...
    <div>
      <Modal open={!!confirmAvance} onClose={() => setConfirmAvance(null)}>
        ...
        <ModalFooter>
          ...
          <Button variant="danger" onClick={executeDeleteAvance}>
            Supprimer
          </Button>
        </ModalFooter>
      </Modal>
      ...
      <Card
        style={{
          marginBottom: 12,
          borderColor: monthBenefice >= 0 ? "var(--green)" : "var(--red)",
        }}
      >
        <CardHeader>
          <CardTitle>
          ...
          <div>
            <div
              style={{
                fontSize: 30,
                fontWeight: 900,
                color: monthBenefice >= 0 ? "var(--green)" : "var(--red)",
              }}
            >
              {formatAr(monthBenefice)}
            </div>
            <div style={{ fontSize: 11, color: "var(--muted)" }}>
              Frais - Salaires - Commission - Recuperations
            </div>
          </div>
          <div style={{ fontSize: 12, color: "var(--muted)", textAlign: "right" }}>
            <div>
              Montant colis : <b style={{ color: "var(--green)" }}>{formatAr(monthTotalMontant)}</b>
            </div>
            <div>
              Frais collectes :{" "}
              <b style={{ color: "var(--orange)" }}>{formatAr(monthTotalFrais)}</b>
            </div>
            <div>
              Salaires agents :{" "}
              <b style={{ color: "var(--red)" }}>{formatAr(monthTotalSalaires)}</b>
            </div>
            <div>
              Commission gerant :{" "}
              <b style={{ color: "var(--pink)" }}>{formatAr(monthGerantGain)}</b>
            </div>
            <div>
              Recuperations :{" "}
              <b style={{ color: "var(--yellow)" }}>{formatAr(monthTotalRecuperations)}</b>
            </div>
      ...
      <Card style={{ marginBottom: 20, borderColor: "var(--purple)" }}>
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 11, color: "var(--muted)" }}>
              {livsGerant(monthLivs).length} livraisons x {formatAr(commissionGerant)}
            </div>
          </div>
        </div>
      </Card>
      ...
      {monthStatsByAgent.map((a) => (
        <Card key={a.id} style={{ marginBottom: 10 }}>
          ...
          >
            <span>
              Total : <b style={{ color: "var(--text)" }}>{a.nbLivs}</b>
            </span>
            <span>
              Livres : <b style={{ color: "var(--green)" }}>{a.nbLivres}</b>
            </span>
            <span>
              Retournes : <b style={{ color: "var(--red)" }}>{a.nbRetours}</b>
            </span>
            <span>
              Reportes : <b style={{ color: "var(--purple)" }}>{a.nbReportes}</b>
            </span>
            <span>
              Frais : <b style={{ color: "var(--orange)" }}>{formatAr(a.totalFrais)}</b>
            </span>
          </div>
          {a.avances.length > 0 && (
              ...
              {a.avances.map((av) => (
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      <span style={{ color: "var(--orange)", fontWeight: 700, fontSize: 13 }}>
      ...
      <Card>
        ...
        <Button variant="warning" fullWidth onClick={handleAddAvance}>
