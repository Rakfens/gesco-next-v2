...
export default function Gerant() {
  const { livraisons, commissionGerant: globalCommissionGerant, updateCommission: onUpdateCommission, showToast } = useApp(); // Supposons updateCommission
  ...
  const { dayLivs, dayCount, dayGain, dayFraisTotal, dayNet, dayExcluded } = useMemo(() => {
    const dayLivs = livsGerant((livraisons as Livraison[]).filter(l => l.date === gerantDate));
    const dayCount = dayLivs.length;
    const dayGain = dayCount * globalCommissionGerant; // Utiliser la commission globale
    const dayFraisTotal = dayLivs.reduce((s, l: Livraison) => s + (typeof l.frais === 'number' ? l.frais : parseFloat(String(l.frais || 0))), 0);
    const dayNet = dayFraisTotal - dayGain;

    const dayExcluded = (livraisons as Livraison[]).filter(l =>
    l.date === gerantDate &&
  ...
  }, [livraisons, gerantDate, globalCommissionGerant]);

  // Calculs pour la vue "mois"
  const { monthLivs, monthCount, monthGain, monthFrais, monthByDay } = useMemo(() => {
    const monthLivs = livsGerant((livraisons as Livraison[]).filter(l => l.date && l.date.startsWith(gerantMonth)));
    const monthCount = monthLivs.length;
    const monthGain = monthCount * globalCommissionGerant; // Utiliser la commission globale
    const monthFrais = monthLivs.reduce((s, l: Livraison) => s + (typeof l.frais === 'number' ? l.frais : parseFloat(String(l.frais || 0))), 0);

    const monthByDay = monthLivs.reduce((acc: Record<string, { date: string; count: number; gain: number }>, l) => {
  ...
  }, [livraisons, gerantMonth, globalCommissionGerant]);
  ...
  return (
    ...
    Commission : {formatAr(globalCommissionGerant)} par livraison (dès que les frais sont payés) {/* Utiliser la commission globale */}
    </p>
    ...
    {editCommission ? (
      ...
      <span style={{ fontSize: 22, fontWeight: 900, color: 'var(--pink)' }}>{formatAr(globalCommissionGerant)}</span> {/* Utiliser la commission globale */}
      <Button variant="ghost" size="sm" onClick={() => { setTmpCommission(globalCommissionGerant); setEditCommission(true); }}> {/* Initialiser tmp avec la globale */}
      Modifier
      </Button>
      </div>
    )}
    ...
    {gerantTab === 'jour' && (
      ...
      {dayExcluded.length > 0 && (
        ...
        {dayExcluded.length} livraison(s) exclue(s) de la commission (clients {EXCLUDED_CLIENTS.join(', ')})
        </div>
        </Card>
      )}
      ...
      <div style={{ fontSize: 32, fontWeight: 900, color: 'var(--text)' }}>{formatAr(dayGain)}</div> {/* Utiliser le gain calculé */}
      <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 4 }}>{dayCount} livraisons × {formatAr(globalCommissionGerant)}</div> {/* Utiliser la commission globale */}
      </Card>
      ...
      <StatCard value={formatAr(dayNet)} label="Frais nets (frais - commission)" color={dayNet >= 0 ? 'green' : 'red'} />
      </div>
      ...
      {dayLivs.length > 0 && (
        ...
        {dayLivs.map((l, i) => (
          ...
          <TableCell align="right" style={{ color: 'var(--orange)' }}>{formatAr(typeof l.frais === 'number' ? l.frais : parseFloat(String(l.frais || 0)))}</TableCell>
          <TableCell align="right">{formatAr(globalCommissionGerant)}</TableCell> {/* Utiliser la commission globale */}
          </TableRow>
        ))}
        </TableBody>
        <TableFooter>
        <td colSpan={6} style={{ padding: '10px 12px', fontWeight: 700, color: 'var(--muted)', fontSize: 11 }}>TOTAL</td>
        <td style={{ padding: '10px 12px', textAlign: 'right', fontWeight: 700, color: 'var(--orange)' }}>{formatAr(dayFraisTotal)}</td>
        <td style={{ padding: '10px 12px', textAlign: 'right', fontWeight: 700, color: 'var(--pink)' }}>{formatAr(dayGain)}</td>
        </TableFooter>
        </Table>
        </div>
      )}
    ...
    {gerantTab === 'mois' && (
      ...
      <div style={{ fontSize: 32, fontWeight: 900, color: 'var(--text)' }}>{formatAr(monthGain)}</div> {/* Utiliser le gain calculé */}
      <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>{monthCount} livraisons × {formatAr(globalCommissionGerant)}</div> {/* Utiliser la commission globale */}
      </div>
      <div style={{ textAlign: 'right', fontSize: 12, color: 'var(--muted)' }}>
      <div>Frais collectés: <b style={{ color: 'var(--orange)' }}>{formatAr(monthFrais)}</b></div>
      <div>Frais nets: <b style={{ color: 'var(--green)' }}>{formatAr(monthFrais - monthGain)}</b></div>
      </div>
      </div>
      </Card>
      ...
      {monthByDay.length > 0 && (
        ...
        <td style={{ padding: '11px 14px', fontWeight: 700, color: 'var(--blue)' }}>{monthCount}</td>
        <td style={{ padding: '11px 14px', textAlign: 'right', fontWeight: 900, color: 'var(--pink)', fontSize: 15 }}>{formatAr(monthGain)}</td>
        </TableFooter>
        </Table>
        </div>
      )}
