...
export default function Historique() {
  const {
    ...
    agents,
    showToast,
    updateLivraison: onUpdateLivraison,
    deleteLivraison: onDeleteLivraison,
  } = useApp();
  ...
  const [fbClient, setFbClient] = useState<string | { client: string; livs: Livraison[] } | null>(
    null,
  );
  const [fbRecup, setFbRecup] = useState<string>("0");
  const [fbProvince, setFbProvince] = useState<string>("0");
  const [confirmDelete, setConfirmDelete] = useState<string | null>(null);

  // États pour les récupérations du jour
  const [_recuperationsJour, setRecuperationsJour] = useState<Recuperation[]>([]);
  ...
  useEffect(() => {
    const _loadRecuperations = async () => {
  ...
  }, [showToast]); // Ajouter getRecuperationsByDate aux dépendances si elle est utilisée
  ...
  const handleExportCSV = () => {
    ...
    const csv = [
      keys.join(","),
      ...livsFiltered.map((l) => keys.map((k) => `"${String(l[k] ?? "")}"`).join(",")),
    ].join("\n");
  ...
  const handleUpdate = async () => {
    ...
    try {
      await onUpdateLivraison(editId!, {
        ...editData,
        montant,
        frais: parseFloat(String(editData.frais)) || 0,
      });
      setEditId(null);
      showToast("Livraison mise à jour");
    } catch (error: unknown) {
      logger.error("Erreur lors de la mise à jour de la livraison:", error);
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      logger.error("Erreur lors de la suppression de la livraison:", error);
  ...
  const totalFrais = useMemo(
    () => livsFiltered.reduce((s, l) => s + (Number(l.frais) || 0), 0),
    [livsFiltered],
  );
    ...
    <div>
        ...
        >
          ...
          {!isMobile && (
            <Select
              ...
              options={[{ value: "tous", label: "Tous" }, ...STATUT_OPTIONS]}
            />
          )}
          <Badge variant="primary">
            {livsFiltered.length} résultat{livsFiltered.length > 1 ? "s" : ""}
          </Badge>
        </div>
        {isMobile && (
      ...
      {livsFiltered.length > 0 && (
        <div>
                    ...
                    {editId === l.id ? (
                      ...
                      >
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setEditData({ ...editData, destinataire: e.target.value })
                          }
                        />
                        <Input
                          ...
                          onChange={(e) =>
                            setEditData({ ...editData, destinataire_lieu: e.target.value })
                          }
                        />
                        <Input
                          type="number"
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setEditData({ ...editData, frais: Number(e.target.value) })
                          }
                        />
                        <Select
                        ...
                        {(editData.statut === "retourne" || editData.statut === "reporte") && (
                          <div style={{ gridColumn: "1/-1" }}>
                            <textarea
                              ...
                              placeholder={
                                editData.statut === "retourne"
                                  ? "Ex: Destinataire absent, adresse incorrecte..."
                                  : "Ex: Reporté au lendemain, client demande de revenir..."
                              }
                            />
                          </div>
                        )}
                        <div style={{ display: "flex", gap: 8, gridColumn: "1/-1" }}>
                          <Button variant="success" onClick={handleUpdate} fullWidth>
                            Sauver
                          </Button>
                      ...
                      >
                        ...
                        >
                          ...
                          <Button variant="danger" size="sm" onClick={() => setConfirmDelete(l.id)}>
                            Supprimer
                          </Button>
                        </div>
                      </div>
          ...
          >
            <span style={{ color: "var(--subtle)", fontWeight: 700, fontSize: 13 }}>
              TOTAL DU JOUR
            </span>
