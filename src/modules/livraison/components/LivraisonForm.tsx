...
export const LivraisonForm = ({ agents, onAddLivraison, showToast, suggestions }: LivraisonFormProps) => {
  ...
  const paiementOptions = Object.entries(PAIE_MODES).map(([key, val]) => ({
    value: key,
    label: val.label,
    icon: val.icon,
  }));
    ...
    <div>
      <div style={{ marginBottom: 20 }} data-testid="livraison-form-page">
        <h1 style={{ fontSize: 22, fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.02em' }}>
          Nouvelle livraison
          {currentCompany && <span style={{ color: 'var(--muted)', fontWeight: 400, fontSize: 14 }}> · {currentCompany.name}</span>}
        </h1>
      </div>
      ...
      <Card padding={isMobile ? 14 : 18}>
        ...
        {/* Client donneur */}
        <div style={{ background: 'var(--blue-dim)', borderRadius: 10, padding: 12, marginBottom: 12 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--blue)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10 }}>
            Client donneur
          </div>
          <Input
        ...
        {/* Destinataire */}
        <div style={{ background: 'var(--teal-dim, rgba(20,184,166,0.08))', borderRadius: 10, padding: 12, marginBottom: 12 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--teal)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10 }}>
            Destinataire
          </div>
          <div style={grid2}>
            <Input
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${paiementObj.length}, 1fr)`, gap: 8 }}>
        ...
        {(form.statut === 'retourne' || form.statut === 'reporte') && (
          <div style={{ marginTop: 12 }}>
            ...
            <textarea
              style={{
                width: '100%', padding: '10px 14px', minHeight: 70,
              ...
              placeholder={form.statut === 'retourne'
        ...
        <div style={{ marginTop: 16 }}>
          <Button variant="primary" fullWidth onClick={handleSubmit} data-testid="btn-submit-livraison">
