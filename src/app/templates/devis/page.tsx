...
export default function DevisTemplatePage() {
  ...
  const resetForm = () => {
    setDevisItems([]); setSearchProduit(""); setShowForm(false);
    ...
    <>
        ...
        <Card className="p-4">
          <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex gap-3">
              <Button onClick={() => { resetForm(); setShowForm(true); }}>+ Nouveau devis</Button>
            </div>
          </CardHeader>
        </Card>
      </div>
                ...
                <TableBody>
                  {devisList.map(d => (
                    <TableRow key={d.id}>
                      ...
                      <TableCell className="text-right font-medium">{formatAr(d.montant_total)}</TableCell>
                      <TableCell>
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${d.statut === "accepte" ? "bg-green-100 text-green-700" : d.statut === "refuse" ? "bg-red-100 text-red-700" : "bg-yellow-100 text-yellow-700"}`}>
                          {d.statut === "accepte" ? "Accepté" : d.statut === "refuse" ? "Refusé" : "En attente"}
                        </span>
                      </TableCell>
                      <TableCell>
                        <div className="flex gap-1">
                          <Button variant="outline" size="sm" onClick={() => printDevis(d)}>Imprimer</Button>
                        </div>
                      </TableCell>
                    </TableRow>
                  ))}
      ...
      {showForm && (
        <Modal open={true} onClose={resetForm}>
          <ModalHeader title="Nouveau devis" subtitle={`${devisItems.length} article(s) · Total: ${formatAr(totalDevis)}`} onClose={resetForm} />
          <ModalBody>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Produits */}
              <div>
                <div className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2">Produits</div>
                <Input type="text" placeholder="Rechercher..." value={searchProduit} onChange={e => setSearchProduit(e.target.value)} className="mb-2" />
                <div className="max-h-64 overflow-y-auto border border-gray-200 rounded-lg p-2 space-y-1">
                  {produitsFiltres.map(p => (
                    <div key={p.id} className="flex justify-between items-center p-2 rounded-lg text-sm hover:bg-gray-50">
                      <div>
                        <div className="font-medium">{p.nom}</div>
                        <div className="text-xs text-muted-foreground">{formatAr(p.prix_vente)}</div>
                      </div>
                      <Button size="sm" onClick={() => addToDevis(p)}>+</Button>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Devis items */}
              <div>
                <div className="text-xs font-bold text-green-600 uppercase tracking-wider mb-2">Articles du devis</div>
                <div className="max-h-40 overflow-y-auto border border-gray-200 rounded-lg p-2 mb-3 space-y-1">
                  {devisItems.length === 0 ? (
                    <div className="text-center text-muted-foreground text-sm py-4">Aucun article</div>
                  ) : devisItems.map(item => (
                ...
                <div className="space-y-2">
                  <div>
                    ...
                    <Input type="text" value={devisInfo.client_nom} onChange={e => setDevisInfo(f => ({ ...f, client_nom: e.target.value }))} />
                  </div>
                  <div>
                    <label className="text-xs font-medium">Téléphone</label>
                    <Input type="text" value={devisInfo.client_telephone} onChange={e => setDevisInfo(f => ({ ...f, client_telephone: e.target.value }))} />
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-xs font-medium">Date</label>
                      <Input type="date" value={devisInfo.date_devis} onChange={e => setDevisInfo(f => ({ ...f, date_devis: e.target.value }))} />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <Input type="text" value={devisInfo.notes} onChange={e => setDevisInfo(f => ({ ...f, notes: e.target.value }))} placeholder="Notes (optionnel)" />
                  </div>
                </div>

                <div className="mt-3 p-3 bg-gray-50 rounded-lg text-sm">
                  <div className="flex justify-between font-bold text-base"><span>TOTAL ESTIMÉ</span><span>{formatAr(totalDevis)}</span></div>
                </div>
              </div>
            </div>
          </ModalBody>
