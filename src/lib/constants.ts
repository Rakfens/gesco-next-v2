// constants.ts — Adapté TypeScript pour Next.js
...
export const STATUTS: Record<string, StatutInfo> = {
...
export const STATUTS_VENTE: Record<string, StatutInfo> = {
...
export const STATUTS_ACHAT: Record<string, StatutInfo> = {
  en_attente: { label: 'En attente', color: '#f59e0b', bg: '#451a03' },
...
// ==================== MODES DE PAIEMENT ====================
export interface PaieModeInfo {
  label: string;
  icon: string;
}

export const PAIE_MODES: Record<string, PaieModeInfo> = {
...
export interface MouvementStockInfo {
  label: string;
  icon: string;
  color: string;
}

export const TYPES_MOUVEMENT_STOCK: Record<string, MouvementStockInfo> = {
...
export const shouldCountGerantCommission = (livraison: {
  ...
  const hasFrais = parseFloat(livraison.frais as string) > 0;
...
export const getCompanyConfig = (slug: string): CompanyConfig => {
